...
export function useVerifyPayment() {
  ...
  return useMutation({
    mutationFn: (reference: string) => apiRequest<{ payment: unknown }>(`/payments/verify/${encodeURIComponent(reference)}`, { method: 'POST' }),
    onSuccess: () => {
...
export function useWalletTopup() {
  return useMutation({
    mutationFn: (input: { amount: number }) =>
      apiRequest<InitializePaymentResult>('/payments/wallet-topup', {
        method: 'POST',
        body: JSON.stringify(input)
      })
  });
...
export function useWallet() {
  return useQuery({
    queryKey: ['wallet'],
    queryFn: () => apiRequest<WalletData>('/payments/wallet')
  });
}
