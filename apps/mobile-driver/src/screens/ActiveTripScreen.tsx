...
export function ActiveTripScreen({ route }: Props) {
  ...
  const latestPoint =
    (driverPoint ?? trip?.trackingPoints?.[0])
      ? {
      ...
      <View style={{ flex: 1, gap: 12 }}>
        <View style={{ gap: 6 }}>
          ...
          <Text style={{ color: theme.colors.muted }}>
            Passenger pickup, route guidance, live GPS, and safety controls.
          </Text>
        </View>
        {MapView && Marker && Polyline ? (
          ...
          >
            ...
            <Text style={{ color: theme.colors.muted }}>
              This runtime does not include the native maps module. Build a development or
              production app to test map rendering.
            </Text>
          </View>
        )}
        <View
        ...
        >
          ...
          <Text style={{ color: theme.colors.muted }}>
            Confirm the passenger, start the trip, and keep location sharing active until drop-off.
          </Text>
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              <Button
                label="Arrived"
                icon={<UserCheck size={18} color={theme.colors.ink} />}
                onPress={() => undefined}
                variant="secondary"
              />
            </View>
