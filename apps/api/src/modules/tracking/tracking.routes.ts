  ...
  asyncHandler(async (req, res) => {
    ...
    const point = await prisma.deliveryTrackingPoint.create({
      data: {
        ...
        ...(typeof req.body.heading === 'number'
          ? { heading: new Prisma.Decimal(req.body.heading) }
          : {}),
        ...(typeof req.body.speedKph === 'number'
          ? { speedKph: new Prisma.Decimal(req.body.speedKph) }
          : {}),
    ...
    req.app
      .get('io')
      ?.to('admins')
      .emit(realtimeEvents.trackingPoint, { ...point, deliveryId });
...
trackingRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    req.app
      .get('io')
      ?.to('admins')
      .emit(realtimeEvents.rideTrackingPoint, { ...point, tripId });
    return ok(res, point);
  })
);
